...
describe('addition of a new note', () => {
  test('succeeds with valid data', async () => {
    ...
    const notesAtAnd = await helper.notesInDb()
    expect(notesAtAnd).toHaveLength(helper.initialNotes.length + 1)

    const contents = notesAtAnd.map(r => r.content)
    expect(contents).toContain(
